...
> fastSolveMazeIter maze target currentPaths = if null inNewList then fastSolveMazeIter maze target newList else reverse (getSecondElemTriple (head inNewList))
>     where newList = [(move dir place, dir: path, move dir place : visited) | (place, path, visited)<-currentPaths, dir<-[N,E,S,W], not (hasWall maze place dir), not (elem (move dir place) visited)]
>           inNewList = filter (\(pl, _, _) -> pl == target) newList
>           getSecondElemTriple (x,y,z) = y
